...
| Tests | **2109 passing** across 32 suites, both builds, zero failures — plus PCA Studio's 86-assertion smoke test (in `./test.sh`) and 169 host-compiled C++ assertions plus **four** sketch-compile checks in `arduino/MaestroPCA/test` (`./run.sh`). v1.45.0 added 227 of them, every one written red first |
...
| Maestro boards | 4 Pololu (Micro 6, Mini 12/18/24) + the MaestroPCA co-processor, **up to eight PCA9685s / 128 channels** (v1.54.0) |
