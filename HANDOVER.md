...
**mod2026**
- **"RAMPING" is not a ramp (mod2026, 2022 BETA).** `if (throttleStickValue -
  driveThrottle < (RAMPING + 1)) driveThrottle += RAMPING; else driveThrottle =
  throttleStickValue;` is backwards: a change bigger than RAMPING jumps to the
  target in one pass, only a change of RAMPING or less is stepped, and a step
  overshoots a smaller change — a stick one unit off flips 9,7,9,7… for as
  long as it sits there (the BETA buzzes ±5 on the Sabertooth wire). The 2025
  sketch fixed the comparison (`> RAMPING`). Reproduced as written and pinned
  in `firmware.test.js` (v1.78.0, review M13).
...
### The 2026-09-01 review — what v1.76.0 closed and what is still open

`docs/REVIEW-2026-09-01.md` is the current list: 5 critical / 10 high / 21
medium / 22 low, twelve of them reproduced against the built files before
they were written down. **v1.76.0 closed H1–H5, H11, H12, M1 and M21** — the
servo seam, PCA Studio's inherited criticals, and the global-scope guard.
**v1.77.0 closed H6–H10, H13–H15, M7, M8 and M16** — the doors that persist or
export. **v1.78.0 closed M2–M6, M9–M15, M18 and the cheap lows (L9, L11, L13,
L14, L16, L17, L19).** **v1.79.0 closed M20, BUILD-01, TOOL-01, TOOL-02,
REL-02 (the repo half), DOC-01, the L21/L22 tooling items and the font
credit.** Still open: M17 (Mike's call); **M19 and the rest of the CI work,
which is a hand-off** — `_stage/ci-handoff-v1.79.0.patch` applies with
`git apply` from the repo root and carries: `check-studio.js` BEFORE
`./build.sh` in both workflows (or the staleness guard can never fail in CI),
`timeout-minutes` on every job, a cache for the Playwright browser, `npm ci`
without its `|| npm install` fallback, and a release step refusing a tag that
does not match `APP_VERSION`; the deep-dive leftovers (DATA-01/02/03,
UX-01/02); and the remaining lows (L1–L8, L10, L12, L15, L18, L20). The
improvements the review sized as M and left — harness helpers and the
`waitForTimeout` retirement (208 calls, ~3 min of the run), a Node test
runner so `npm test` needs no bash, the persistence-state indicator, the
"Setup snapshot" rename and a portable bundle, pausing the renderer under an
opaque overlay — are in `docs/REVIEW-2026-09-01.md` §Improvements. The OFL
licence text for IBM Plex is not in the tree; the credit says so, and where
it goes is Mike's call. One v1.78.0 choice worth knowing: turning the
AstroPixels layer off now UNMOUNTS and frees the rigs rather than hiding them,
so off-and-on-again costs one dome fit (a few ms) — `apxSync` can set
`rig.visible=false` instead if the toggle should be free. Two things
v1.77.0 deliberately left, both one line if wanted: the cued take's nested
"· strings" sequence stays sparse on the channels no string plays (it is a
brick INSIDE its routine, where the compiler supplies the base; dense rows
there would clobber the routine's other bricks — the new `pulses-off` lint
warns if it is ever put on the board alone), and a saved Studio project that
boot had to repair is repaired in memory and re-logged on every load until
the first edit saves it. M17 — the header chip connecting and
driving every boot-ticked channel home with no warning — is Mike's call and
is left as it is.
...
- **A fix in one host's copy of a shared duty is not a fix.** Found by the
  2026-09-01 review: v1.69.0 §1.1 (`rebuild` dropping `aim`), §1.3 (Finish
  never sending the rate frame) and v1.70.1's half-typed-width refusal were
  each fixed in the sim's implementation of the `HW` seam — `hw-host.js`,
  `setup-hw-channels.js` — and never in PCA Studio's `44-hw-host.js`,
  `30-project.js` and the shared `hw-table.js`. Three releases after a rename
  keystroke stopped walking a driven servo to its stop in the sim, it still
  did in Studio. §3's rule ("anything the two hosts agree on belongs in the
  shared module, not the seam") is the cure, and v1.76.0 applied it: the carry
  is `pcaCarryState()` in the engine, the config-frame push is
  `serialCfgSync()` in the serial layer, and a host cannot omit either. When a
  seam duty is fixed, port it to the other host AND move it down.
- **A constant a shared module reads has to be as shared as the module.**
  `PCA_MAX_BOARDS_UI` lived in `maestro/boards.js` (sim only) and was read by
  `setup-hw.js` (both apps), so Studio's "PCA9685s" wizard step threw
  `ReferenceError` and rendered blank from v1.69.0 to v1.75.1 — in front of a
  green smoke test that stepped 0→2→3→0→3→4 and never rendered step 1. Same
  shape as `escGuard` (v1.42–1.44) and `download()`. `tools/check-globals.js`
  now fails `test.sh` on any free identifier in a Studio-loaded file that only
  the sim manifest declares (a `typeof NAME` guard EARLIER in the file, or a
  `host-optional` comment on the line, is how a file says "I know"), and
  `servo-units.js` — the file both manifests load first — is where a shared
  constant goes. And a `const` a script reads at PARSE time must be declared
  in an EARLIER script: moving the constant meant moving `servo-units.js`
  above `boards.js` in the sim manifest, or `boards.js` died in the TDZ.
...
> **One version (v1.79.0).** `APP_VERSION` in `src/js/core/util.js` is the
> version. `./build.sh` (or `npm run build`) copies it into `package.json` and
> `package-lock.json`'s root entries, so a release bump is one edit and the
> build; commit those two with it. The release workflow's tag gate (in the
> CI hand-off) refuses `vX` unless `APP_VERSION` is `X`.
...
- **Two GitHub Actions** run on that remote: every push builds and runs the
  complete suite against both app targets plus PCA Studio's smoke test. A `v*`
  tag repeats the tests, then attaches the simulator, PCA Studio, the offline
  manual and its quick references, and every Arduino sketch pack named by
  `arduino/packs/make-packs.sh`. `tools/check-packs.js`, run by `test.sh`, makes
  that builder list the release workflow's contract: every built pack must be
  attached and announced, and no attached pack may be absent from the builder.
  So a release is `git tag v1.45.0 && git push --tags`, not a file uploaded by
  hand. Both workflows are pinned to the node24 action runtime (v1.44.1); if
  the deprecation warnings come back, bump the majors rather than silencing
  them.
...
### 2026-09-01 - v1.79.0: the tooling

The last batch of the 2026-09-01 review: the things that decide whether a
green run means anything.

**BUILD-01 — the two main targets had no staleness guard.** `dev.html` and
`R2D2-Simulator.html` are gitignored, so git cannot notice them going stale,
and every suite loads whatever bytes are on disk; the 2026-08-23 review ran
the estate against a 1.74.1 dist beside 1.75.0 sources and chased
missing-global "bugs" until somebody rebuilt. `tools/check-build.js` is
`check-studio.js` for both targets — the real generator with its two writes
intercepted, compared in memory, naming the module at the first difference —
and `test.sh` runs it before the suites. It found a stale dist on its first
run here (agent R had just edited `about.js`). CI is unaffected: it builds
first, so the check passes there; the laptop is where it matters.

**REL-02 — one version.** `package.json` said 1.44.1 through thirty-odd
releases and was `private:false` with empty `homepage`/`repository` fields.
`APP_VERSION` in `util.js` is the version; `tools/build.js` now copies it into
`package.json` and the lockfile's two root entries whenever they differ and
says so, `private` is `true`, and `check-build.js` swallows those two writes
so a check writes nothing. The tag↔version gate for the release workflow is
in the CI hand-off.

**TOOL-01 — the npm entry points work from a Windows shell.** `npm run build`
calls the two Node builders directly (no `./build.sh` dispatch, which
PowerShell and cmd cannot run); `npm test` runs `bash test.sh`, which needs
a bash on the path (Git Bash or WSL) — a Node runner that needs none is the
remaining improvement, and `test.sh`'s status-capture logic (v1.70.0) is why
it was not rewritten in a hurry.

**test.sh bounds every suite.** A `page.evaluate` that never resolves used to
hold the script for ever and a CI runner for six hours; `run_suite` wraps
each suite in `timeout 900` where coreutils `timeout` exists (Git Bash has
it) and reports a kill as a FAIL line rather than silence.

**TOOL-02 and M20 — `run.sh`.** The native firmware tests needed `zip` and
`unzip` for their last step and said only g++; a box without them now stops
at the top with one line saying which is missing, and `test/README.md` says
the same. The AddressSanitizer run reported through `cmd && echo PASS`, which
`set -e` does not fail on when the command is not last in the list — an ASan
report (what found §8.1) could print and the run go green; it is two
statements now, and there was no other line of that shape.

**L22 — `make-packs.sh`, the vendor script, two test pages.** One `fail`
flag across the pack loop meant the first bad pack silently skipped every
good pack after it while the zips already written stayed in `dist/` under a
"nothing written"; packs are judged independently and a failing run removes
`dist/`. `tools/build.js` now `esc()`s the inlined vendor script too — three.js
carries no `</script` today and the day a bump does, the dist truncates at
parse time. `build-config.test.js`'s `bootPage` and `wiring.test.js`'s
`tzPage` had no `pageerror` listener; each collects and asserts its own.

**DOC-01 / L21 — measured, not remembered.** README: twenty-one chapters and
eight clips everywhere (it disagreed with itself six lines apart), ≈8.9 MB
and 486 KB from `ls -l`, fifteen stylesheets from the manifest, thirty-eight
suites with the eight missing rows added to the table from each suite's own
header, `cd r2d2-astromech-simulator`, "the build fails" (it exits 1 — "a
warning nobody reads is not a check"), and an honest sentence about
`waitForTimeout` instead of "never". IBM Plex Mono and Sans (© IBM, SIL OFL
1.1) join CREDITS.md and the About card — the OFL text itself is not in the
tree, which both say.

**Deliberately not done.** The angled-include glob in `run.sh` still scans
`*.ino` only: widening it to `*.h` would fail Mike's own run on the untracked
`sequencesold.h`, which is the one file it would catch. The CI hunks are a
hand-off (§6). The AstroPixels off-toggle costs a dome fit (v1.78.0's
choice) — unchanged.

### 2026-09-01 - v1.78.0: the mediums

The third batch of the 2026-09-01 review (`docs/REVIEW-2026-09-01.md`), by
five agents on disjoint files; every fix pinned red on v1.77.0 first. Grouped
by what it protects.

**The dome lights (M2, M3, M4, L19).** The build's Dome lighting answer only
reached `APX.on` through `apxInit()`, which the wizard's tick never called, so
choosing Teeces on the setup step left the LogicEngine running until a reload
while the Model pane still said AstroPixels; `buildApply()` now re-runs
`apxInit()` whenever the answer differs from the one the boards were built
against (`APX.domeLights`), and the Model pane's tick stopped calling it a
second time. The rigs, the dome fit and the textures had no way out of their
cache: the layer going off left both rigs on the dome in front of the stand-in's
own fittings, a model swap re-parented the MK4's rig and fit onto the Polar
Mouse (`cad.test.js` had been doing exactly that swap, unasserted), and a
board-variant change tested three `LR` fields that never existed and kept the
old texture size. One `lrReset()` (render3d.js) serves all three; the CAD rig
is keyed on `CAD.header`. `apxSend()` never read the `jawa` flag it declared,
so `@1T3` on the Standard sketch put the logics on ALARM — refused now with the
sketch named, and the command box no longer offers it there. `apxLE` gated
speed and colour one digit late on 3- and 4-digit bodies; the pane's refusal
toast wore the green edge. `lights.test.js` +8, `cad.test.js` +1.

**The engine and its clock (M5, M18).** `pcaseq.js` clamped a tick at 200 ms
where `MaestroPCA.cpp` (widened in v1.69.0) does not — one 250 ms call stepped
20 in the twin and 25 in the firmware — and did not release a displaced
routine's per-frame speeds where the C++ `restartScript()` always has, so
Studio's preview of a displacing routine ran the newcomer at the old routine's
pace. Both match the C++ line for line now; the cap is the CLOCK's job. And
the clock itself threw away the fraction of every fire (`dtms|0` downstream):
a `setInterval(10)` that really fires every 10.7 ms lost 1.94 % over 300
fires, so every PCA_Bridge ramp finished late by that margin. `hwClockFire()`
advances `last` by the whole milliseconds it delivered and carries the rest;
the 250 cap keeps its meaning. `pcaseq.test.js` +8 (parity block from the C++
tests' numbers), `hw.test.js` +3.

**The sequencer's two readings of one timeline (M6, M15).** `blockPoseAt()`
and `blockFreeAt()` started every channel closed and read only the bricks
whose window contained the instant, while `blockCompile()` carries a channel's
last value between intervals — so an `'o'` brick followed by any later brick
compiled OPEN and scrubbed SHUT, and an unwired `'o'` brick closed on the model
during pad playback while a wired one held. Both now carry (`blockCarryAt`),
by the compiler's own rule. Delete, Backspace and Ctrl+Z reached the sequencer
through the servo bench overlay because the key guards listed the surfaces by
hand and `#setupWrap` was not among them; they ask `uiModalOpen()` now.
`sequencer.test.js` +7, `sequencer-ui.test.js` +7.

**Lint that cried wolf (M9).** `chan-range` made every reversed channel an
ERROR ("Fix the endpoints in Control Center") although REV is recorded by
swapping the pair, so every export of a reversed channel said "Written with N
validation errors outstanding"; only `min === max` is an error now, and a
Maestro build gets a warning that Control Center wants min ≤ max. `timing`
judged every re-target from rest, and a compiled ramp re-targets before each
step arrives by design, so one plain `oc` brick at Mike's 80/10 drew three
warnings on a routine that arrives on time; a run of same-direction re-targets
is one move now, judged when something reverses it. `export-guards.test.js`
+9.

**Firmware fidelity (M10, M11, M12, M13).** The transpiler's `map()`
truncated the SUM where Arduino truncates the quotient and adds — the exact
off-by-one `util.js map_` was cured of in v1.69.0 — so half the stick was one
unit hot on every imported sketch; it IS `map_` now, and the A/B golden runs
full reverse at tolerance 0. Editing a constant of an imported sketch changed
nothing until the profile reloaded (the closure read `__cfg` once); a config
edit on a sketch is a re-flash now (`sketchReflash()`), said so in the log.
`mixHubDrive()` passed a float to `map_` where the sketch's `long map()`
truncates first — 16 % of stick positions differed by one; `Math.trunc` on
both speeds, and the test oracle had the same omission. And "RAMPING is not a
ramp" — mod2026 and the 2022 BETA test `< (RAMPING + 1)`, so a big change
JUMPS and a one-unit change flips 9,7,9,7 for ever; the 2025 sketch fixed it —
is now a `notes` entry on both profiles (§4 of this file has it too) and
`firmware.test.js` asserts the real shape instead of a comment claiming a
climb in steps of 2. Nothing simulated changed. `sketch.test.js` +11,
`profiles.test.js` +3, `firmware.test.js` +5.

**Input and the kiosk (M14, L11, L13, L14).** The Guide+LB+RB chord called
`xboxDisconnect()` in Sim only, and the only way back was a chip on the hidden
header; the chord is ignored while `kioskOn()`. The S/T TIMEOUT chip did not
fire for a starved pure turn and printed `delay(750)` for any block; it fires
on turn too and prints the real length. An RC Advanced `act:` binding rested a
self-centring channel at half travel and `rcRestWarnings` could not see it;
it maps from the channel's rest point like a trigger and the warning reads the
delivered value. The on-screen stick click was a 60 ms wall-clock window a
slow frame skipped; it is a `virtualPress()`. `CFG.servoSpeed` was clamped at
the Config box only, so a `.json` could deliver 0 or a negative; clamped at
the use site like `maestroRate`. `kiosk.test.js` +4, `rc.test.js` +8,
`chrome.test.js` +4.

**Copy and the small screens (L9, L16, L17).** The header chip and the ⚡
button said "a board running PCA_Bridge" for a Pololu Maestro and a
MaestroReplacement too — connected, the words come from `SER.kind` and the
banner; idle, from what the build says a click would open. Studio's "32
channels = two boards — the bridge stops there" was five releases stale. The
library footer names the first group actually below the fold rather than
READY-MADE when READY-MADE is on screen. Four strings sent people to "the
Maestro tab", gone since v1.73.0. The wizard chip tooltip said "Step 7 of 15"
beside a footer saying "Question 7 of 9"; one counter, derived. The part card
survived a model switch and floated over the servo gauges with a live Port
select — `modelApply()` deselects. Toasts fired under the wizard landed in
`#stage` behind the opaque overlay and expired unseen — the servo-import
receipt among them; while `uiModalOpen()` the host hangs off `<body>`.
`select.test.js` +5, `look-boards.test.js` +2, `workspaces.test.js` +10.

### 2026-09-01 - v1.77.0: the doors that persist or export

The second batch of the 2026-09-01 review (`docs/REVIEW-2026-09-01.md`):
everything that writes to storage, to a file that gets flashed, or to the
library — done by four agents on disjoint files, every fix with an assertion
watched red on v1.76.0 first.

**H6 — an imported `.ino` that throws bricked every boot.** The identifier
accounting deliberately ignores method names, so a sketch calling one library
method its adapter lacks (`mp3.playFolder(1,2)` on the MD_YX5300 shim)
transpiled clean, was stored and made the build's firmware, and THEN ran —
`loadProfile(bootFw)` threw inside the boot handler on every reload after,
nothing past it ran, and the fix was clearing localStorage. Three fences now,
in the order a bad sketch meets them: `sketchTrial()` runs `setup()` and three
`loop()` passes at the drop door BEFORE anything is stored (a throw refuses
the file by method, and the firmware that was running is re-flashed because
the trial ran real shims against the live droid); the sketch profile's own
`setup()`/`loop()` wrappers hand a later throw to `fwFallback()`
(core/firmware.js), which loads the setup's recommendation, points the build
back at it AND unpins it, and names the method — on the sketch profile only,
not in `fwLoop()`, so the three hand ports keep throwing loudly; and main.js
wraps the boot `loadProfile()` the same way. `sketch.test.js` +20, including
a `page.reload()` on a stored bad sketch.

**H7 — a strings-only puppet take switched servos OFF on a real Maestro.**
The take was a change-only frame list with 0 = "leave it alone", which the
sim honours and `genSeqBody` does not: a Maestro target of 0 is *stop sending
pulses*, so a held panel went limp at the next keyframe and every unmapped
servo was switched off at frame 0. `pupDensify()` now seeds frame 0 with
every servo-mode channel's closed end and carries through `cueDensify()`;
`puppet.test.js` parses the generated script back and proves no channel is
ever sent 0 after being driven. Lint gained `pulses-off` (warn) for any
sequence that still does — hand-edited `.mstr` files included.

**H8 — a nested library sequence compiled speeds above the channel's own.**
`chanSpeedForMs()` capped only at Pololu's 16000 and `blockCompile()` sized a
speed to every interval, including those a `seq` brick commanded: a 100 ms
full-throw frame went to the board as Set Speed 224 on a speed-120 channel.
The inverse is capped at `c.speed` (0 still unlimited), and an interval a
`seq` brick commands carries the nested frame's own speed or 0; durations are
untouched (`ramp-step.test.js`).

**H9 — re-importing your own `.mstr` bound subs to the wrong routine** when
two names shared a `niceName()` symbol (`Dome Wave`/`Dome_Wave`, or the `·`
a merge mints): `_2` was numbered in loadout order going out and library
order coming back. `parseScriptSubs()` reads the `# <name>` line `genScript()`
has always written above each `sub` and binds on the exact name first,
symbol second (`roundtrip.test.js`, a reversed loadout of four colliders).

**H10 — one gate for every channel row.** `chanNormalise()` (servo-cfg.js):
finite integers in the protocol's 0..16383, modes and homemodes from their
enums, ease from `EASE_KINDS`, speed/acceleration/releaseMs at the firmware's
widths, names strings. Used by the servo-config door, the whole-setup door,
the browser store at boot, the wizard's replace and — because it has three
callers — inside `mstrApply()` itself. Whole rows that cannot be read become
the padding row; partial rows keep what the table had; every receipt counts
the repairs and the log names them. Before it, `min:"abc"` sent a 16000
target to the wire and `min:"4000"` concatenated targets. `mstrParse()` also
refuses NaN from a hand-edited attribute on its own.

**H14 — the whole-setup import committed the prefs half first.** It assigned
PREFS, saved, applied the theme and model, and only then touched MSTR with no
shape check, so a bad Maestro block left the previous build answers gone and
the table half-written. `setupImportRead()` checks the entire file into a
plan; PREFS and MSTR commit in one stretch; a refusal names the field and
says "nothing was changed" (`setup.test.js`, with localStorage asserted
unchanged). This door also rebuilds the bench engine after replacing the
table wholesale, which it never had.

**H15 — "save existing, then replace" emptied the library first.** The
adopter now writes into a working list and yours is restored on a throw, and
the toast says so (`maestro-import.test.js`).

**H13 — PCA Studio's project door saved before it checked.** `projNormalise()`
(30-project.js) builds a fresh object; the load handler adopts, saves and
rebuilds only once it exists and says in words what it repaired; boot runs the
saved text through the same gate, parks anything unusable under
`pcastudio.bad` byte for byte, and starts from the default project rather
than a blank page (`smoke.test.js` +12, including `page.reload()` on the
review's `{channels:{}}` blob). Studio 0.13.1.

**M7, M8, M16.** A name or act duplicated on either side no longer
identifies a channel during adoption (those rows go by number). The
choreography `.json` keeps its bricks at 250/750/1000 ms (`stepMs` sits beside
`blocks`, not on it). The music builder goes through `seqUniqueName`; the
Board pane's Rename refuses a taken name in the library card's own words; ⟶M
regenerates a same-named pair in place so the d-pad's slot plays the current
mapping (it will overwrite a hand-edited routine of that name, logged with
its brick count — Mike's call whether that should ask first).

### 2026-09-01 - v1.76.0: the seam is shared

Mike, 2026-09-01: *"review the Sim and identify any improvements you could
make"*, then *"go for fixing"*. The review is `docs/REVIEW-2026-09-01.md` —
five area reviewers, then every finding that moves a servo, loses work or
bricks the app reproduced against the built files before it was written up.
This is the first of its batches: the servo seam, and PCA Studio's copy of it.

**What was wrong.** Three of v1.69.0's hardware criticals were still in PCA
Studio, because each fix had landed in the sim's implementation of the `HW`
seam and nowhere else:

- `30-project.js`'s `rebuildEngine(true)` carried `target` and not `aim`, so
  every keystroke in Studio's channel table rebuilt an engine that steered
  every driven servo to its home — 0 on a `homemode:'Off'` channel, which
  pins the horn at `c.min`. Reproduced: `aim 7000 → 0`, position
  `6998, 6917, 6711, 6380, 5923…` after one letter of a rename. (§1.1)
- `44-hw-host.js`'s `applied()` never sent the config frame, so a 200 Hz rate
  chosen on Studio's wizard streamed 200 Hz tick counts at a board still at
  50 Hz — 1500 µs asked for, 6 ms emitted, port still open. (§1.3)
- `hw-table.js` wrote `min`/`max`/`home` to the engine — and the wire — on
  every `input` event with `min="300" max="2700"`: typing 1500 wrote 1, 15
  and 150 µs to a real servo first. The bench's own Configure panel had
  refused this since v1.70.1; Studio's table had not.

And two things the sim had never had right:

- **The link-row rate box and Finish disagreed.** `serialSetFreq()` wrote
  `HW.freq()` and the board, but neither the bench's copy of the setup
  (`SETUP.hw`, taken at `setupOpen`) nor `HWCFG`, the record `hwCfgPush()`
  compared against. Finish wrote the old rate back, the compare read
  50 == 50, and the resync streamed 50 Hz tick maths at a 200 Hz prescaler:
  reproduced as `307` ticks queued for a 1500 µs channel — 375 µs on that
  board — and then `setupClose()` closed the port with the horn on its stop.
- **The dial could not reach past the stored ends on the engine path.**
  `calDrive()` widened `c.min`/`c.max` around one `HW.drive()`, and the
  comment beside it said `pcaStepChannel` never re-clamps. It clamps the
  position every tick, so the target was accepted and the servo walked back
  to the stored end — unless nothing had ever driven the channel, in which
  case it snapped through. First turn works, second does not: "flaky", and
  re-measuring a narrowed channel over PCA_Bridge was impossible.
- `HW.rebuild(true)` did not carry `known`, so a servo parked by `releaseMs`
  snapped on its next command after any bench edit; and speed, acceleration
  and sleep typed in Configure reached the table and the save but not the
  engine, so the "Move it now" slider kept the old speed while a paced
  Maestro took the new one.

Plus the one that had thrown on every click for four releases: Studio's
"PCA9685s" wizard step read `PCA_MAX_BOARDS_UI` from `maestro/boards.js`,
which Studio does not load — `ReferenceError`, empty card, since v1.69.0's
own §3.6 fix put the constant there. `blockSeqDur()` reached `seqTotal()` in
`playback.js` the same way, so dropping a whole-sequence brick in Studio
threw and added nothing.

**What changed, and where the duty lives now.**

- `pcaseq.js` — **`pcaCarryState(old, E, channels)`**: the one state carry
  (`active`, `pos256`, `vel256`, `target`, `aim`, `known`, `free`; servo on
  both sides; clamped into the new ends when active). `hw-host.js` and
  `30-project.js` both call it and keep no copy. **`pcaBounds(c, s)`**: every
  clamp in the engine reads it; it is the channel's own pair unless the
  dial's window `st.free` is open — null on every path but the dial's, so
  the firmware parity is untouched (the C++ has no dial).
- `serial-link.js` — **`SER.sent`** `{port, freq, osc}`, written by both
  frame writers (`serialConfig`, `serialSetFreq`) and cleared on disconnect;
  **`serialCfgSync()`**: off, config frame, forget the tick cache — if and
  only if `SER.sent` disagrees with `HW`. `hwCfgPush()` is now that call;
  `HWCFG`/`hwCfgSeen()` are gone. Studio's `applied()` calls it too.
- `hw-ui.js` — the rate box writes `SETUP.hw.freq` while the bench is open.
- `setup-hw-cal.js` — `calDrive()` opens `st.free` (`calFree(ch, true)`)
  instead of widening the table; `setupCalLeave/Cancel/Commit` shut it, and
  shutting it on a servo parked past the ends brings its aim back inside so
  it RAMPS home. The comment that said the step never re-clamps says what
  actually happened.
- `setup-hw-channels.js` — every Configure field but the name rebuilds.
- `hw-table.js` — the same `pwEndFault`/`pwEndsRefusal` gate as the bench,
  `PW_ABS` as the boxes' own min/max, `pwCentreFollow` on an end edit.
- `servo-units.js` — `PCA_MAX_BOARDS_UI` lives here (both manifests load it
  first); `boards.js` reads it. `src/manifest.json` lists `servo-units.js`
  BEFORE `boards.js` now, because `boards.js` reads the constant at parse
  time and a later script's `const` is in the TDZ.
- `blocks.js` — `blockSeqDur()` sums the frames itself.
- `setup-hw.js`, `serial-link.js`, `setup-hw-channels.js` — three sim-only
  helpers that were guarded by a `typeof` on a DIFFERENT name
  (`servoMcuOpt`, `buildCoprocBoards`, `buildSet`, `domeMapKeys`) are guarded
  by their own.
- **`tools/check-globals.js`** (new, in `test.sh`): tokenises both manifests,
  collects depth-0 declarations, fails on a duplicate name and on any free
  identifier in a Studio-loaded file that only the sim manifest provides,
  unless the file asks `typeof NAME` earlier or the line says
  `host-optional`. Proven red on the reintroduced `seqTotal`. It reported
  2,134 sim names, 385 Studio names, no duplicates.
- `pca-studio/smoke.test.js` — renders every `SETUP_STEPS` entry, types in the
  Boards field, drops a whole-sequence brick, pins the `aim` carry, the Finish
  config frame and the typed-width refusal, and **asserts page errors at the
  end** (it printed the count and asserted only at load). 86 → 101.
- `tests/hw.test.js` (+13), `tests/pcaseq.test.js` (+9). Every new assertion
  was run against the untouched v1.75.1 tree first: the H1 block produced the
  `307`-tick frame, the Studio block produced `max: 4 → 60 → 600`.

**Deliberately not done here:** the rest of the review (§6 above says which).
M17 — connecting from the header chip commands every boot-ticked channel home
with no warning — is left as it is; it is defensible as "connect = power-up"
and it is Mike's call.

### 2026-08-23 - release pack guard: what is built is what ships

The release workflow built three Arduino packs and attached two. `Esp32Droid`
compiled in CI, appeared in `make-packs.sh`'s `PACKS` source-of-truth list and
was linked prominently from the README, but the workflow dropped its zip
before the release and did not name it in the release body. Every check was
green because they proved that the pack could be made, not that it shipped.

`tools/check-packs.js` now parses the builder's own `PACKS` block rather than
copying a fourth list. For every pack it requires both an exact matching zip
in the release workflow's `files:` block and its exact backticked filename in
the release body; in the other direction, an attached pack that the builder
no longer makes fails too.
`test.sh` runs that guard beside the generated-file check and captures Node's
status before doing anything else with its output. Proven red on the previous
workflow as both `BUILT BUT NOT ATTACHED — Esp32Droid` and `ATTACHED BUT
UNANNOUNCED — Esp32Droid`, then green with all three named.

The release body also catches up with the manual itself: twenty-one chapters
and eight screen-capture clips. `APP_VERSION` stays 1.75.1 because none of this
changes the app. On the first release tag after this lands, confirm that
GitHub actually publishes `Esp32Droid.zip`; the guard proves what the workflow
declares, not what GitHub ultimately serves.

### 2026-08-23 - v1.75.1: a name was executable

A review on 2026-08-23 said user and imported names could execute. The first
thing done with that claim was to try it, because a security finding you have
not reproduced is a rumour:

```
Model Builder part name: <img src=x onerror="window.__x=1">        fired = 1
PCA sequence name:       <img src=x onerror="window.__pca=1">      fired = 1
```

Both now fire `0`. That is not a cosmetic markup bug: the injected script
shares the page context that holds the Web Serial handle to a servo board,
and can read and rewrite the whole in-memory project. Calibrate the threat
honestly, though — it is not drive-by. Somebody has to get a hostile name
into your project, which means a shared `.mstr`, a project JSON, a dropped
`.r2m`, or a typed name. Swapping config files IS the workflow in this hobby,
which is why it matters.

**`sect()` takes text nodes, not an escaper.** It is the heading 64 call
sites share and `right` is the slot most of them use for a name. An escaper
is a thing the next caller can forget; a text node cannot be talked into
being markup. Every call site was read first: **not one passes a tag**, so
nothing was lost. Exactly one passed an HTML ENTITY — `Speed &amp; feel`,
added in v1.75.0 — and it is a plain ampersand now, which is what it meant.

**The imported-sketch trap is the one worth remembering.** `PROFILE.file`,
`.short` and `.blurb` look like constants, and are, for the three hand ports.
For an imported `.ino` they are the FILENAME the user dropped
(`profiles/sketch-import.js`), and they reached the sketch blurb, the
pinned-firmware line, the Maestro pane and both "these do not line up" lists.
A value is not constant because it is constant on the paths you happened to
walk.

Also fixed: both export receipts (the `.mstr` filename, and lint messages
that quote channel and sequence names — escaped inside `exportLintNote()`,
the one place they are built, rather than at each of the two printers); a sub
name parsed out of an imported `<Script>` with `/sub\s+(\S+)/`, where `\S+`
happily matches a whole tag; the `.r2m` header’s `kind`, which
`cadHeaderCheck` never validated; and the calibration dial’s channel name.

**That last one has its own escaper on purpose.** `setup-hw-cal.js` is in
BOTH manifests, and `xmlEsc()` lives in `maestro/boards.js`, which PCA Studio
does not load. Reaching for it there would have thrown a `ReferenceError` and
blanked the dial in Studio only — the kind of bug nobody finds.

**In Studio the sequence tabs are DOM nodes**, not a string, so escaping is
enforced by the DOM rather than by the next person to edit the line. The
three `style="background:…"` sinks are **validated**, not escaped: escaping
the quote closes the attribute break and leaves you arbitrary CSS, so
`blkColor()` output must match `/^#[0-9a-fA-F]{3,8}$/` or it falls back.

**`tests/hostile-names.test.js` is new, and red-first: 11 of its 14
assertions fail against the build immediately before this commit.** Two
things about how it is written are deliberate. Every payload is an EVENT
HANDLER, never a `<script>` tag — a script inserted with `innerHTML` does not
execute, so a suite written with `<script>alert(1)</script>` passes against a
completely vulnerable app and reports that it is safe. And each case asserts
BOTH that nothing ran AND that the name is still on screen literally, because
a sink that silently drops the name would pass the first half while losing
the user’s label. Two assertions exist only to prove the other two are not
passing against an empty string — the calibration dial and the part card each
have to have actually drawn first. Both of those caught a vacuous pass while
this file was being written.

**77 suite runs, all green.**

Still open from the same review, and NOT in this commit: **REL-01**, the
`Esp32Droid.zip` that `make-packs.sh` builds and `README.md` links and
`release.yml` does not attach. It is a workflow file, which the desktop
bridge refuses to write, so it went to Codex as a pack with the patch, a
tested `tools/check-packs.js` guard and both red/green proofs.
