...
Still open, in the order the review suggests: **H6–H10, H13–H15** (the doors
that persist or export: the sketch that bricks the boot, the puppet take that
switches servos off on a Maestro, the nested speed above the channel's limit,
the sub-name collision on re-import, the unvalidated JSON channel fields, the
persist-before-validate imports); then the mediums (M2–M6, M9–M20); then the
tooling (BUILD-01, TOOL-01, REL-02, DOC-01, TOOL-02) and the deep-dive
leftovers (DATA-01/02/03, UX-01/02). M17 — the header chip connecting and
