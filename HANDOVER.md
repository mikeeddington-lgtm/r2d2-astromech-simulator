...
| Tests | **2096 passing** across 32 suites, both builds, zero failures — plus PCA Studio's 86-assertion smoke test (in `./test.sh`) and 157 host-compiled C++ assertions plus three sketch-compile checks in `arduino/MaestroPCA/test` (`./run.sh`). v1.45.0 added 227 of them, every one written red first |
