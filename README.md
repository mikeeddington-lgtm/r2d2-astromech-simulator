...
<p align="center">
  ...
  <sub>Your host sketch needs no changes: same library, same slot numbers, same three bytes<br>
  An Arduino sketch pack · unzip, edit <code>Config.h</code>, upload · one library from Library Manager and nothing else to download<br>
  <a href="arduino/MaestroPCA/examples/MaestroReplacement/README.md">the step-by-step</a></sub><br>
  <br>
  <a href="https://github.com/mikeeddington-lgtm/r2d2-astromech-simulator/releases/latest/download/Esp32Droid.zip">📡 <strong>Esp32Droid</strong></a>
  — sixteen servos straight off an ESP32's own pins, no expander<br>
  <sub>0.305 µs a step, better than a PCA9685 and close to a Maestro · no I2C, no address jumpers, no second board<br>
  <b>Not yet run on real hardware</b> — it compiles on both arduino-esp32 cores and every check that does not need silicon passes<br>
  <a href="arduino/MaestroPCA/examples/Esp32Droid/README.md">the step-by-step</a></sub>
</p>
...
[`pca-studio/PCA-Studio.html`](pca-studio/PCA-Studio.html) — because the whole
promise of that folder is *open this one file from disk*, which a clone should
keep without needing a build first. Its own smoke test runs against it, so a
stale copy fails loudly rather than quietly.

### R2 Bench Console

**[`arduino/bench-sketches/R2_Bench_Console/`](arduino/bench-sketches/R2_Bench_Console/README.md)**
— an Arduino you type at over the serial monitor, so you can ask the servo
layer for anything Padawan360 would ask it **without a Padawan360 ADK, a USB
Host Shield, an Xbox receiver or a droid**. That is what it is for: it is a way
of not needing the rest of the stack while you prove one wire.

It talks to a real Pololu Maestro, a MaestroReplacement or PCA9685 boards with
the same commands, so a difference in behaviour between two rigs is a
difference in the *hardware* rather than in the test. Every read is done
against a deadline, so a board that is not wired to answer is **reported**
silent rather than hanging the sketch — which is not true of Pololu's own
library, and is most of why this exists.

Download it as a ready-to-run pack from the
[latest release](https://github.com/mikeeddington-lgtm/r2d2-astromech-simulator/releases/latest/download/R2_Bench_Console.zip),
or open the folder from a clone.

### The Arduino side

**[`arduino/`](arduino/)** — the `MaestroPCA` library (a Maestro's sequence
engine on a PCA9685, with its JS twin kept integer-for-integer identical in
`pcaseq.js`), the `MaestroReplacement` and `Esp32Droid` sketches built on it,
and `PCA_Bridge` in `pca-studio/`, which is the first sketch most people flash.

`arduino/MaestroPCA/test/run.sh` compiles all of it for the host and is run on
every push. The three downloadable packs are generated by
`arduino/packs/make-packs.sh` from the sketch folders themselves — there is no
second copy in the repository to go stale.
